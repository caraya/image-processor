...
let jxlEncoderReady: Promise<void> | null = null

/**
 * Loads and initializes the JXL WASM module explicitly for Node runtimes.
 */
async function ensureJxlEncoderReady(): Promise<void> {
  if (!jxlEncoderReady) {
 ...
 */
function logVerboseErrorDetails(err: unknown, verbose?: boolean): void {
  if (!verbose) return
 ...
 */
function createSharpInstance(filePath: string, verbose?: boolean): sharp.Sharp {
 ...
 */
function makeNumberedOutputPath(filePath: string): string {
 ...
 */
function ensureOutputDoesNotClobberInput(outputPath: string, inputPath: string, verbose?: boolean): string {
  ...
  for (const format of formats) {
    ...
    if (format === 'jpegxl') {
      ...
      try {
        ...
        if (verbose) console.log('📦 Encoding JPEG XL using @jsquash/jxl (WASM)')
        await ensureJxlEncoderReady()
        const jxlBuffer = await encodeJxl(imageDataLike as any, {
...
program
  ...
  .action(async (source: string, options: {
    ...
    const qualityOverrides: FormatQuality = {
